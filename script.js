...
async function toggleMic() {
    ...
    if (permission.state === 'denied') {
        alert('Microphone access is denied. Please allow access to use this feature.');
        return;
    }

    if (recognizing) {
        // Stop recognition for Android and Windows
        if (isDesktop()) {
        ...
        // Implement stopping for iOS/macOS
        else if (isIOSOrMac()) {
            // Stop the sfspeech recognition here
            sfspeech.stop(); // Assuming you have a stop method in sfspeech
            recognizing = false;
            document.getElementById('micButton').textContent = 'Start Listening';
            listeningAnimation.style.display = 'none'; // Hide animation
        }
    } else {
        // Start recognition for Android and Windows
        if (isDesktop()) {
            recognition.start();
            recognizing = true;
        ...
        // Implement starting for iOS/macOS
        else if (isIOSOrMac()) {
            // Start the sfspeech recognition here
            sfspeech.start(); // Assuming you have a start method in sfspeech
            recognizing = true;
            document.getElementById('micButton').textContent = 'Stop Listening';
            listeningAnimation.style.display = 'block'; // Show animation
        }
...
// Helper function to check if the device is iOS or macOS
function isIOSOrMac() {
    const userAgent = window.navigator.userAgent;
    return /iPad|iPhone|iPod|Macintosh/.test(userAgent);
}

// Helper function to check if the device is desktop (Windows)
function isDesktop() {
    const userAgent = window.navigator.userAgent;
    return /Windows|MacIntel/.test(userAgent) && !/Mobile/.test(userAgent);
}
